...
//			
			List<Integer> ids = Arrays.asList(10000,2,6460,5403);
//			System.out.println(" ----------------- IN ------------------");
//			List<Document> studentList = myCollection.find(in("student_id", ids)).into(new ArrayList<>());
//			studentList.forEach(x->System.out.println(x.toJson(prettyPrint)));
//			
			System.out.println(" ----------------- AND ------------------");
			List<Document> studentList = myCollection.find(and(eq("last_name", "Tordjman"), in("student_id", ids))).into(new ArrayList<>());
			studentList.forEach(x->System.out.println(x.toJson(prettyPrint)));
